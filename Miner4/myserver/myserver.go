...
// End 接收传送完毕信号，组装并验证新区块，如果验证成功则加入区块链，同时要删除所有同时包含在新块和交易池中的交易
func (*Server) End(ctx context.Context, in *proto.EndRequest) (*proto.EndResponse, error) {
	...
	return &proto.EndResponse{
		ER: "Miner4:Block " + strconv.FormatInt(block.Index, 10) + " received,verification " + judge,
	}, nil
}
